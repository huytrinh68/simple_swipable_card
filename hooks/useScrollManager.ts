...
export const useScrollManager = (routes: { key: string; title: string }[], sizing = Theme.sizing) => {
  ...
  useEffect(() => {
    const listener = scrollY.addListener(({ value }) => {
      ...
      tabkeyToScrollPosition[curRoute] = value; // {"all": 489, "gainers": 240, "losers": 924}
    });
    return () => {
      ...
      Object.keys(tabkeyToScrollableChildRef).forEach((key) => {
        ...
        if (/* header visible */ key !== curRouteKey) {
          if (scrollValue <= CBTabViewOffset + sizing.header) {
            // console.log('scrollValue | CBTabViewOffset + sizing.header', key, scrollValue, CBTabViewOffset + sizing.header)
            console.log('isShowing and ', key, Math.max(
            ...
            console.log('tabkeyToScrollPosition[key]', key, tabkeyToScrollPosition[key])
            console.log('isHidden and ', CBTabViewOffset + sizing.header)
            scrollRef.scrollToOffset({
